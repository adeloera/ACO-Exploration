...
#Here I download the ACO level datasets with more detailed program paramenters from the CMS website and perform some basic cleaning. 
...
aco_data_2017 <- read_csv("https://data.cms.gov/api/views/gk7c-vejx/rows.csv?accessType=DOWNLOAD",
  ...
  select(aco_num, aco_name, aco_state, initial_start_date, n_ab, sav_rate, bnchmk_min_exp, gen_save_loss, earn_save_loss, qual_score, updated_bnchmk, a_btot_bnchmk, a_btot_exp, final_share_rate)%>%
  
  #Next I reformat some of the columns as necessary, beginning by setting the intital start time variable as a date and creating a "length of service" variable. 
  
  mutate(initial_start_date = as.Date(initial_start_date, format = "%m/%d/%Y"),
         service_length = Sys.Date()-initial_start_date) 
...
aco_data_2015 <- read_csv("https://data.cms.gov/api/views/7rrf-3gxr/rows.csv?accessType=DOWNLOAD",
  ...
  select(aco_num, aco_name, aco_state, start_date, n_ab, sav_rate, bnchmk_min_exp, gen_save_loss, earn_save_loss, qual_score, updated_bnchmk, a_btot_bnchmk, a_btot_exp, final_share_rate) %>%
